...
namespace Core.Geom
{
    ...
    /// </summary>
    public class Polygon2 : Shape2, ISpatial
    {
        /// <summary>
        ...
        /// <example>
        /// <para>var poly = ShapeUtils.CreateRectangle(2, 3);</para>
        /// <para>Console.WriteLine($"corner-to-corner = longest axis = Math.Sqrt(2 * 2 + 3 * 3) = {Math.Sqrt(2 * 2 + 3 * 3)} = {poly.LongestAxisLength}");</para>
        /// </example>
        /// </summary>
        ...
        public static bool Intersects(Polygon2 poly1, Polygon2 poly2, Vector2 pos1, Vector2 pos2, Rotation2 rot1, Rotation2 rot2, bool strict)
        {
            if (rot1 == Rotation2.zero && rot2 == Rotation2.zero)
            {
                ...
                for (int i = 0, len = poly1.Normals.Count; i < len; i++)
                {
                    ...
                    var proj1 = ProjectAlongAxis(axis, pos1, Rotation2.zero, poly1.Center, poly1Verts);
                    var proj2 = ProjectAlongAxis(axis, pos2, Rotation2.zero, poly1.Center, poly2Verts);
